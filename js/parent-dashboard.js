...
const standardsMap = { math:['4.NBT.B.5','4.NBT.B.6','4.NF.A.1'], ela:['4.RL.KID.2','4.L.CSE.1'], science:['4.ESS1.1','4.LS2.2'], social:['4.19','4.10'] };
...
window.switchPlannerSubject = (subj) => { currentPlannerSubj = subj; renderPlanner(); };
...
function loadPlannerForm() {
  ...
  let html = `<div style="display:flex; gap:10px; margin-bottom:15px; flex-wrap:wrap;">
    <input type="text" id="plannerQuote" value="${saved.quote||''}" placeholder="Quote of the week..." style="flex:2; padding:8px; border-radius:8px; border:2px solid #e2e8f0;">
    <input type="text" id="plannerAuthor" value="${saved.author||''}" placeholder="Author" style="flex:1; padding:8px; border-radius:8px; border:2px solid #e2e8f0;">
  </div>`;
  ...
  days.forEach(d => {
    ...
    dayData.standards.forEach(s => {
      html += `<div class="planner-row">
        <select class="std-code"><option value="">Select</option>${(standardsMap[currentPlannerSubj]||['Custom']).map(x=>`<option value="${x}" ${x===s.c?'selected':''}>${x}</option>`).join('')}</select>
        <input type="number" value="${s.w}" style="width:60px;">
        <button onclick="this.parentElement.remove()" style="background:#ff6b6b; color:white; border:none; padding:4px 8px; border-radius:8px;">✕</button>
      </div>`;
    });
    ...
    dayData.urls.forEach(u => {
      html += `<div class="planner-row"><input type="url" value="${u}" placeholder="URL" style="flex:1;"><button onclick="this.parentElement.remove()" style="background:#ff6b6b; color:white; border:none; padding:4px 8px; border-radius:8px;">✕</button></div>`;
    });
...
window.addPlannerRow = (day, type) => {
  ...
  if(type==='std') row.innerHTML = `<select class="std-code"><option value="">Select</option>${(standardsMap[currentPlannerSubj]||['Custom']).map(x=>`<option value="${x}">${x}</option>`).join('')}</select><input type="number" value="20" style="width:60px;"><button onclick="this.parentElement.remove()" style="background:#ff6b6b;color:white;border:none;padding:4px 8px;border-radius:8px;">✕</button>`;
  else row.innerHTML = `<input type="url" placeholder="URL" style="flex:1;"><button onclick="this.parentElement.remove()" style="background:#ff6b6b;color:white;border:none;padding:4px 8px;border-radius:8px;">✕</button>`;
  btn.parentElement.insertBefore(row, btn);
};

window.savePlanner = () => {
  const data = { quote: document.getElementById('plannerQuote')?.value || '', author: document.getElementById('plannerAuthor')?.value || '' };
  document.querySelectorAll('.planner-day').forEach((dayEl, idx) => {
    const d = days[idx];
    const stds = Array.from(dayEl.querySelectorAll('.std-code')||[]).map(sel => ({c: sel.value, w: sel.parentElement.querySelector('input[type=number]')?.value||20}));
...
window.addMockGrade = () => {
  const name = prompt('Assignment name:'); const subj = prompt('Subject:').toLowerCase(); const score = parseInt(prompt('Score (0-100):'));
  if(name && subj && !isNaN(score)) {
    const grades = JSON.parse(localStorage.getItem('mock_grades') || '[]');
    grades.push({ id: Date.now(), name, subject: subj, score: Math.min(100, Math.max(0, score)), date: new Date().toLocaleDateString() });
    localStorage.setItem('mock_grades', JSON.stringify(grades)); renderGradebook();
  }
};
window.deleteGrade = (id) => {
  if(!confirm('Delete this grade?')) return;
  let grades = JSON.parse(localStorage.getItem('mock_grades') || '[]');
  grades = grades.filter(g => g.id !== id); localStorage.setItem('mock_grades', JSON.stringify(grades)); renderGradebook();
};
...
function renderRewards() {
  ...
  const rewards = [{name:'Candy Bar', cost:500, icon:'🍫'},{name:'Roblox ($10)', cost:2000, icon:'🎮'},{name:'Movie Night', cost:750, icon:'🎬'},{name:'Art Supplies', cost:500, icon:'🎨'}];
  sec.innerHTML = `
    <h3 style="margin-bottom:15px;">🎁 Reward Manager</h3>
    <p style="text-align:center; font-size:1.2rem; margin-bottom:20px;">Current Balance: <strong style="color:#667eea;">${pts} points</strong></p>
    <div class="reward-grid">${rewards.map(r => `<div class="reward-card"><div class="icon">${r.icon}</div><h4>${r.name}</h4><p class="cost">${r.cost} pts</p><button class="redeem-btn" onclick="redeemReward(${r.cost}, '${r.name}')">Redeem</button></div>`).join('')}</div>
  `;
}
...
function renderSettings() {
  ...
  sec.innerHTML = `
    ...
    <div class="settings-card"><p><strong>📎 Export Progress:</strong> Download all data as CSV.</p><button onclick="exportCSV()" class="action-btn">Download CSV</button></div>
    <div class="settings-card" style="border-color: #ff6b6b;"><p><strong>🗑️ Reset All:</strong> Erase points, grades, plans. Cannot be undone.</p><button onclick="resetAllData()" class="danger-btn">Reset All Data</button></div>
  `;
}
window.exportCSV = () => {
  let csv = 'Subject,Assignment,Score,Date\n';
  JSON.parse(localStorage.getItem('mock_grades') || '[]').forEach(g => csv += `${g.subject},${g.name},${g.score}%,${g.date}\n`);
  csv += `\nTotal Points,${localStorage.getItem('totalPoints')||0}\nQuizzes Completed,${localStorage.getItem('quizzesCompleted')||0}`;
  const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv], {type:'text/csv'})); a.download = 'alli_progress.csv'; a.click();
};
window.resetAllData = () => { if(confirm('⚠️ Erase ALL data?')) { localStorage.clear(); location.reload(); } };